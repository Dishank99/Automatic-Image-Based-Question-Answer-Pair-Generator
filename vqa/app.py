...
def get_image_features(image_file_name, image_model):
...
def main():
    ...
    for label in reversed(y_sort_index[0,-5:]):
        # print(label)
        label_name_api_resp = requests.post('http://127.0.0.1:5000/',data = json.dumps({'label':str(label)}), headers={'Content-Type': 'application/json'})
        label_name = json.loads(label_name_api_resp.content)['label_name']
        # print(label_name)
        # print( str(round(y_output[0,label]*100,2)), "% ", label_name)
        print("{}% {}".format(round(y_output[0,label]*100,2), label_name))
...
def get_answer(image_model, vqa_model, image, question, global_result):

    print("\n\n\nLoading image features ...")
    image_features = get_image_features(image, image_model)    
    ...
    for label in reversed(y_sort_index[0,-5:]):
        ...
        # print( str(round(y_output[0,label]*100,2)), "% ", label_name)
        # print("{}% {}".format(round(y_output[0,label]*100,2), label_name))
        predicted_result.append(str(label))
    global_result.append(predicted_result)
...
def home():
    ...
    question_array = request.json['question_array']

    image_paths = [each_path[1:-1].replace('////','/') for each_path in image_path_array[1:-1].split(', ')]
    questions = [each_question[1:-1] for each_question in question_array[1:-1].split(', ')]

    # return jsonify({'image_paths':image_paths, 'questions':questions})

    print('Getting image model...')
    ...
    for image, question in zip(image_paths, questions):
        get_answer(image_model, vqa_model, image, question, global_result)
